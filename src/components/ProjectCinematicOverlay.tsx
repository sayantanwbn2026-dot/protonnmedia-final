...
const ProjectCinematicOverlay = ({ project, isOpen, onClose, showCaseStudyLink = true }: Props) => {
  ...
  useEffect(() => {
    ...
    if (contentRef.current) {
      gsap.fromTo(
        ...
        { y: 20, opacity: 0 },
        { y: 0, opacity: 1, stagger: 0.05, duration: 0.4, delay: 0.2, ease: 'power3.out' }
      );
    }
  }, [isOpen, project]);
      ...
      >
        ...
        <div ref={contentRef} className="p-8 pt-16">
          ...
          <div className="mb-6">
            ...
            {project.director && (
              <span className="font-body text-xs text-muted-foreground mt-1 block">{project.director}</span>
            )}
          </div>
          ...
          <div className="pt-4 border-t border-border/10 flex flex-wrap gap-4">
            {showCaseStudyLink && project.slug && (
              <Link
                ...
                data-cursor-hover
                className="inline-flex items-center gap-1 font-body text-xs text-primary border-b border-primary/40 hover:border-primary transition-colors"
                onClick={handleClose}
              >
...
export default ProjectCinematicOverlay;
